    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Login(string email, string password)
        {
            ...
            if (user != null)
            {
                ...
                {
                    var ptnt = db.PatientTables.Where(u => u.UserID == user.UserID).FirstOrDefault();
                    Session["Patient"] = ptnt;
                }
               ...
               if (NewPassword == ConfirmPassword)
                {
                    ...
                    //db.Entry(user).State = EntityState.Modified;
                    //db.SaveChanges();
                    ViewBag.message = "Change Successfuly";
                    return RedirectToAction("Login", "Home");
        ...
        public ActionResult CreateUser(UserTable user)
        {
            if (user != null)
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        ViewBag.Message = "Email Already Registered  , Please Enter Profile Detail";
                        ...
                        else if (finduser.UserTypeID == 4) // PATIENT
                        {
                            ...
                            var ptnt = db.PatientTables.Where(u => u.UserID == finduser.UserID).FirstOrDefault();
                            if (ptnt == null)
                            {
                                return RedirectToAction("AddPatient");
                            }
